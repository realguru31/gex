...
def create_price_chart(ticker, gex_levels=None, spot=None):
    """Plotly candlestick: dodgerblue up, magenta down. Full trading day."""
    price_df = fetch_price_data(ticker, n_bars=100)
    ...
    # Format index to HH:MM strings for clean x-axis
    time_labels = []
    for idx_val in price_df.index:
        if hasattr(idx_val, "strftime"):
            time_labels.append(idx_val.strftime("%H:%M"))
        else:
            time_labels.append(str(idx_val))
    ...
    fig.update_layout(
        title=dict(text=f"{display_ticker} Intraday", font=dict(color=CS["text"], size=13)),
...
def create_positions_chart(calls_df, puts_df, spot, percent_range=0.03):
    """
    Horizontal bar chart like the attached image:
      Put OI  = orange bars going LEFT (negative)
      Call OI = dodgerblue bars going RIGHT (positive)
      Spot    = yellow horizontal line
    """
    fig = go.Figure()
    ...
    c_oi = oi["call_oi"].values
    p_oi = oi["put_oi"].values

    # Put OI goes LEFT (negative), Call OI goes RIGHT (positive)
    ...
    # Spot line (yellow horizontal) — use add_shape for categorical y-axis
    # Find nearest strike label to spot
    nearest_idx = int(np.argmin(np.abs(strikes - spot)))
    spot_y = f"${strikes[nearest_idx]:.0f}"
    fig.add_shape(
        type="line", x0=-max(p_oi.max(), 1), x1=max(c_oi.max(), 1),
        y0=spot_y, y1=spot_y, yref="y",
        line=dict(color=CS["gold"], width=3),
    )
    fig.add_annotation(
        x=max(c_oi.max(), 1) * 0.8, y=spot_y,
        text=f"Spot ${spot:.2f}", font=dict(color=CS["gold"], size=10),
    ...
    fig.update_layout(
        ...
        xaxis=dict(gridcolor=CS["grid"], title="Open Interest",
                   zeroline=True, zerolinecolor=CS["grid"], zerolinewidth=1),
        yaxis=dict(gridcolor=CS["grid"], title="",
                   categoryorder="array",
                   categoryarray=[f"${s:.0f}" for s in strikes]),
        barmode="overlay",
        legend=dict(bgcolor=CS["plot_bg"], bordercolor=CS["grid"],
                    font=dict(size=8), orientation="h",
                    yanchor="bottom", y=1.02, xanchor="center", x=0.5),
        margin=dict(l=55, r=10, t=35, b=30),
        height=500,
        bargap=0.15,
    )
    return fig
...
if snap is not None:
    ...
    with col_price:
        ...
        st.plotly_chart(price_fig, width="stretch", key="price_chart")
        ...
        st.plotly_chart(pos_fig, width="stretch", key="pos_chart")

    with col_gex:
        # Chart 1: Gamma Density
        st.plotly_chart(snap["fig1"], width="stretch", key="gex_chart_1")
        # Chart 2: Charm Density
        st.plotly_chart(snap["fig2"], width="stretch", key="gex_chart_2")
        # Chart 3: Charm Pressure & Acceleration
        st.plotly_chart(snap["fig3"], width="stretch", key="gex_chart_3")
