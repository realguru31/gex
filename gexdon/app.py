...
def create_price_chart(ticker, gex_levels=None, spot=None):
    ...
    if gex_levels:
        level_styles = {
            ...
            "gamma_zero_gamma":       ("Zero Γ",    "#ffffff",    "dash"),
...
def create_positions_chart(calls_df, puts_df, spot, percent_range=0.03):
    ...
    # Spot line (yellow horizontal)
    spot_label = f"${spot:.0f}"
    # Find nearest strike to spot for y position
    fig.add_hline(y=spot_label, line=dict(color=CS["gold"], width=3),
...
if snap is not None:
    ...
    with col_price:
        ...
        st.plotly_chart(pos_fig, use_container_width=True, key="pos_chart")

    with col_gex:
        # Chart 1: Gamma Density
        st.plotly_chart(snap["fig1"], use_container_width=True, key="gex_chart_1")
        # Chart 2: Charm Density
        st.plotly_chart(snap["fig2"], use_container_width=True, key="gex_chart_2")
        # Chart 3: Charm Pressure & Acceleration
        st.plotly_chart(snap["fig3"], use_container_width=True, key="gex_chart_3")
