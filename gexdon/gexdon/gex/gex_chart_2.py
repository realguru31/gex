...
Inputs : data dict from fetch_options_data(), percent_range
Outputs: (matplotlib Figure, dict of key charm levels)
"""
import numpy as np
import pandas as pd
...
def generate_charm_chart(data, percent_range=0.03):
    ...
    for K in all_K:
        cc, pc = 0.0, 0.0
        cd = calls[calls["strike"] == K]
        if not cd.empty:
            r = cd.iloc[0]
            iv = resolve_iv(r["impliedVolatility"], fb_iv)
            if iv and r["openInterest"] > 0:
                _, _, ch = bs_greeks(spot, K, t_exp, R, iv, "call")
                cc = ch * r["openInterest"] * 100
        pd_ = puts[puts["strike"] == K]
        if not pd_.empty:
            r = pd_.iloc[0]
            iv = resolve_iv(r["impliedVolatility"], fb_iv)
            if iv and r["openInterest"] > 0:
                _, _, ch = bs_greeks(spot, K, t_exp, R, iv, "put")
                pc = ch * r["openInterest"] * 100
    ...
    ax.axvline(spot, color=CS["cyan"], ls="--", lw=2, label=f"Spot ${spot:.2f}", alpha=.8)
    _style(ax, st, fig, f"Charm Density ({exp_lbl})", fb_iv)
    return fig, levels


def _style(ax, strikes, fig, title, fb_iv):
    ...
    iv_tag = "Yahoo IV" if fb_iv is None else f"Est IV {fb_iv:.3f}"
    ax.set_title(f"{title} — {iv_tag}", color=CS["text"], fontsize=12, fontweight="bold")
