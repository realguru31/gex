...
Uses Barchart pre-computed gamma when available, BS fallback otherwise.
"""
import numpy as np
import pandas as pd
...
def generate_gex_chart(data, percent_range=0.03):
    ...
    _style(ax, st, fig, f"Gamma Density ({exp_lbl})", fb_iv, source)
...
def _style(ax, strikes, fig, title, fb_iv, source):
    ...
    ax.set_title(f"{title} — {iv_tag} [{src_tag}]", color=CS["text"], fontsize=12, fontweight="bold")
